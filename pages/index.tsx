...
const Home: NextPage = () => {
  ...
  const filterByBrand = (filteredData: any) => {
    ...
    const filteredCars = filteredData.filter(
      (car: any) => car.name.split(" ").indexOf(selectedBrand) !== -1
    );
    return filteredCars;
  };
  
  const filterByYear = (filteredData: any) => {
    ...
    const filteredCars = filteredData.filter(
      (car: any) => car.release_year === selectedYear
    );
    return filteredCars;
  };
  ...
  const handleYearChange = (event: any) => {
    const inputYear: any = Number(event.target.id);
